...
export class AppController {
  ...
  async chips() {
    ...
    try {
      const response = await axios.get(
        `${process.env.CHIPS_API}/Datamart_COREProg_dbo_vw_report_CoreProg_EmployeeData/?$top=3`,
        {
          headers: {
  ...
  async training() {
    ...
    try {
      const response = await axios.get(
        `${process.env.CHIPS_API}/Datamart_COREProg_dbo_vw_report_CoreProg_LearningData/?$top=1`,
        {
          headers: {
            'x-cdata-authtoken': process.env.CHIPS_API_KEY,
          },
