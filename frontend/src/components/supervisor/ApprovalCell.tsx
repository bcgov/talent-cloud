...
export const ApprovalCell = ({
  ...
  const handleSubmitDenial = async (values: {
    ...
    const reason =
      values.reason === SupervisorReason.OTHER
        ...
        : values.reason;
    try {
      await AxiosPrivate.patch(`/recommitment/${personnel.id}`, {[program]:{
  ...
  const disabled =
    (program === Program.BCWS &&
      personnel.bcws &&      personnel.recommitment?.find(itm => itm.program === Program.BCWS)?.status !== RecommitmentStatus.MEMBER_COMMITTED &&
      personnel.recommitment?.find(itm => itm.program === Program.BCWS)?.status !== RecommitmentStatus.SUPERVISOR_DENIED) ||
    (program === Program.EMCR &&
      personnel.emcr &&
      personnel.recommitment?.find(itm => itm.program === Program.EMCR)?.status !== RecommitmentStatus.MEMBER_COMMITTED &&
      personnel.recommitment?.find(itm => itm.program === Program.EMCR)?.status !== RecommitmentStatus.SUPERVISOR_DENIED);
      ...
      <div className="flex flex-row gap-x-4 pr-12">
        <select
          disabled={
            disabled 
            ||
            personnel.recommitment?.find(itm => itm.program === program)?.status === RecommitmentStatus.SUPERVISOR_DENIED
          }
          className={[
